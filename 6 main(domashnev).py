...
from stage7 import build_rag_context
...
def run_graph_rag_pipeline(user_query: str):
    ...
    if qdrant_results:
        ...
        ]) СТАРОЕ, АДАПТИРУЕТСЯ в LIST ДЛЯ STAGE7'''

        qdrant_context = [ #НОВОЕ, АДАПТИРУЕТСЯ в LIST ДЛЯ STAGE7
            {
                "text": r['text'],
                "source": f"{r['source']}, стр. {r['page']}",
                "score": r['score']
            }
            for r in qdrant_results
        ]
    ...
    # Сборка контекста
    context = build_rag_context(qdrant_context, graph_text, 3000, 0.01, 0.01, 0.8, 0.8)
    # Конец сборки контекста

    # ЭТАП 4: Генерация финального ответа
    print("\n[4/4] Генерация финального ответа через DeepSeek...")
    final_answer = generate_final_answer(user_query, context) # graph_text Изменён на context
